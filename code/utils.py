...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, length):
...
def convert_examples_to_features(examples, label_list, max_seq_length,
                                 tokenizer, cls_token_at_end=False, pad_on_left=False,
...
class WordSubstitute(object):
    def __init__(self, perturb):
        ...
        self.perturb_key = set(list(perturb.keys()))
        self.exclude = set(string.punctuation)

    def get_perturbed_batch(self, batch, rep=1):
        # rep is substitution numbers
        num_text = len(batch)
        out_batch = []
        for k in range(rep):
            for i in range(num_text):
                ...
                if tem_text[0]:
                    # try to substitude each word
                    for j in range(len(tem_text)):
                        if len(tem_text[j]) > 0 and tem_text[j][-1] in self.exclude:
    ...
    def sample_from_table(self, word):
        if word in self.perturb_key:
            tem_words = self.perturb[word]['set']
